...
export default function IndustriesMaster() {
  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("master_industries" as any).select("*").order("sort_order");
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        const { error } = await supabase.from("master_industries" as any).update(form).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("master_industries" as any).insert(form);
        if (error) throw error;
      }
  ...
  const del = async (id: string) => {
    const { error } = await supabase.from("master_industries" as any).delete().eq("id", id);
    if (error) {
      if (error.code === "23503") {
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Lead industry classifications (IT, Manufacturing, Retail…)</p>
        </div>
        <Button onClick={openNew}><Plus className="h-4 w-4 mr-1" />Add Industry</Button>
      </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editing ? "Edit Industry" : "Add Industry"}</DialogTitle></DialogHeader>
