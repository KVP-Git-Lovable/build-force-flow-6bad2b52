...
export default function LeadSourcesMaster() {
  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("master_lead_sources" as any).select("*").order("sort_order");
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        const { error } = await supabase.from("master_lead_sources" as any).update(form).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("master_lead_sources" as any).insert(form);
        if (error) throw error;
      }
      qc.invalidateQueries({ queryKey: ["lead-sources-admin"] });
  ...
  const del = async (id: string) => {
    const { error } = await supabase.from("master_lead_sources" as any).delete().eq("id", id);
    if (error) {
      if (error.code === "23503") {
        await supabase.from("master_lead_sources" as any).update({ is_active: false }).eq("id", id);
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Where leads come from (Event, Referral, Website…)</p>
        </div>
        <Button onClick={openNew}><Plus className="h-4 w-4 mr-1" />Add Source</Button>
      </div>
