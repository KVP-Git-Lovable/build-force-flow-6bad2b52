...
export default function PaymentTermsMaster() {
  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("master_payment_terms" as any).select("*").order("sort_order");
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        const { error } = await supabase.from("master_payment_terms" as any).update(form).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("master_payment_terms" as any).insert(form);
        if (error) throw error;
      }
      qc.invalidateQueries({ queryKey: ["payment-terms-admin"] });
      qc.invalidateQueries({ queryKey: ["master-payment-terms"] });
      setOpen(false);
      toast.success("Saved");
  ...
  const del = async (id: string) => {
    const { error } = await supabase.from("master_payment_terms" as any).delete().eq("id", id);
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage payment terms used in opportunities</p>
        </div>
        <Button onClick={openNew}><Plus className="h-4 w-4 mr-1" />Add Term</Button>
      </div>

      <Card><CardContent className="p-0 overflow-x-auto">
        <Table>
          <TableHeader><TableRow>
            <TableHead>Order</TableHead><TableHead>Name</TableHead><TableHead>Active</TableHead><TableHead></TableHead>
