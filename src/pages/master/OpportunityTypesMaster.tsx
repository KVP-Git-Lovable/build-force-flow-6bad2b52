...
export default function OpportunityTypesMaster() {
  const nav = useNavigate();
  const qc = useQueryClient();
  const { data: rows = [] } = useQuery({
    queryKey: ["opp-types-admin"],
    queryFn: async () => {
      const { data, error } = await supabase.from("opportunity_types").select("*").order("sort_order");
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        const { error } = await supabase.from("opportunity_types").update(form).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("opportunity_types").insert(form);
        if (error) throw error;
      }
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Opportunity Types</h1>
          <p className="text-sm text-muted-foreground">Manage the types available in the opportunity form</p>
        </div>
